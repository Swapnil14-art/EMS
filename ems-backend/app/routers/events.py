...
from app.models.event import (
    ...
    EventCoordinator, EventEditHistory,
)
...
async def list_events(
    ...
    if not current_user:
        ...
        # Base: events they created or coordinate (any status)
        ...
        if current_user.department_id:
            ...
            conditions.append(Event.club_id.in_(dept_clubs.scalar_subquery()))
...
async def get_event(
    ...
    if can_view_internal_docs(current_user):
        data["documents"] = [
...
async def submit_event(
    ...
    try:
        ...
        clashing = await check_venue_clash(
            db, event.venue_id, event.venue_custom,
        ...
        if clashing:
            raise HTTPException(
                status_code=409,
                detail={
                    "message": "Venue clash detected. An approver must override to proceed.",
                    "clashing_events": [{"id": e.id, "title": e.title} for e in clashing],
                },
            )
...
async def cancel_event(
    ...
    if not can_cancel_event(current_user, event):

        ...
        "clashing_events": [
            {
                ...
                "status": e.status,
                "venue_names": [v.name for v in getattr(e, 'venues', [])] if getattr(e, 'venues', None) else ([e.venue.name] if getattr(e, 'venue', None) else [e.venue_custom or "Unknown"]),
            }
            for e in clashing
        ],
    }
...
async def delete_venue(
    ...
    result = await db.execute(select(Event).where(Event.venue_id == venue_id).limit(1))
    if result.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Cannot delete venue. Please delete the events under that venue first.")

    child_result = await db.execute(select(Venue).where(Venue.parent_id == venue_id).limit(1))
    if child_result.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Cannot delete venue. Please delete or reassign its child venues first.")

...
import { eventService, venueService, clubService } from '@/lib/services';
...
const schema = z.object({
  ...
  school_department: z.string().min(1, 'Department required'),
...
export default function CreateEventForm({ basePath }: { basePath: string }) {
  ...
  const STEP_FIELDS: Record<number, (keyof FormData)[]> = {
    1: ['title', 'event_type', 'school_department', 'event_incharge_name', 'event_incharge_contact', 'target_audience', 'objectives', 'collaborating_club_ids', 'sponsor_name'],
  ...
  const saveAsDraft = async (data: FormData) => {
    ...
    try {
      const payloadData = { 
        ...
        venue_custom: data.venue_type === 'Other' ? data.venue_custom : null,
        venue_id: data.venue_type !== 'Other' ? data.venue_id : null,
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="page-subtitle">Fill in all sections to submit your event for approval</p>
        </div>
      </div>

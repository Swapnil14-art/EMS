...
const schema = z.object({
  ...
  code: z.string().min(2, 'Department code required').max(10, 'Code too long'),
});
...
export default function AdminDepartmentsPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to delete department (it might be in use)');
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div><h1 className="page-title">Department Management</h1><p className="page-subtitle">{total} listed departments</p></div>
        <Button icon={<Plus className="w-4 h-4" />} onClick={() => setCreateOpen(true)}>Create Department</Button>
      </div>

      <div className="card p-4 flex gap-3">
        <Input placeholder="Search departments…" leftIcon={<Search className="w-4 h-4" />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? Array.from({ length: 6 }).map((_, i) => (
          ...
          <div className="col-span-full"><EmptyState icon={<Building2 />} title="No departments found" action={<Button onClick={() => setCreateOpen(true)}>Create First Department</Button>} /></div>
        ) : filtered?.map(dept => (
      ...
      <Modal open={createOpen} onClose={() => { setCreateOpen(false); reset(); }} title="Create Department"
        footer={<>

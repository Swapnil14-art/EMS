...
export default function AdminUsersPage() {
  ...
  const handleCreate = async (data: CreateForm) => {
    setSubmitting(true);
    try {
      await userService.preApprove({ email: data.email, role: data.role, department_id: data.department_id });
      toast.success('User pre-approved — they can sign up with this email later');
      setCreateOpen(false);
      reset();
      fetchUsers();
    } catch (err: any) {
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="secondary" icon={<Users className="w-4 h-4" />} onClick={openBulkPanel}>Bulk Manage Students</Button>
          <Button icon={<Plus className="w-4 h-4" />} onClick={() => setCreateOpen(true)}>Create User</Button>
        </div>
      </div>
      ...
      <Modal open={createOpen} onClose={() => { setCreateOpen(false); reset(); }} title="Create New User"
        ...
        <div className="space-y-4">
          <Alert type="info">
            <span>This pre-approves the email for the specified role. When they sign up later, they will automatically be assigned this role and department.</span>
          </Alert>
          ...
          <Input label="Department ID (leave blank for Director/Admin)" type="number"

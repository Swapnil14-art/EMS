                  ...
                  <div className="border border-dashed border-[var(--input-focus-ring)] bg-white rounded-xl p-4 text-center">
                    ...
                    {selectedFile ? (
                      <div className="space-y-3">
                        <div className="p-3 bg-violet-50 border border-violet-100 rounded-lg flex flex-col items-center">
                          <FileText className="w-6 h-6 text-violet-600 mb-1" />
                          <p className="text-sm font-medium text-violet-900 truncate max-w-full px-2">{selectedFile.name}</p>
                          <button onClick={() => { setSelectedFile(null); if (fileRef.current) fileRef.current.value = ''; }} className="text-xs text-violet-600 hover:underline mt-1">Change File</button>
                        </div>
                        <Button loading={uploading} onClick={handleUpload} className="w-full justify-center h-11 bg-violet-600 hover:bg-violet-700 text-white border-0" icon={<CheckCircle2 className="w-4 h-4" />}>
                          SUBMIT RnD REPORT
                        </Button>
                      </div>
                    ) : (

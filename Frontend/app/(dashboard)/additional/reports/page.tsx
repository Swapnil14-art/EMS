...
import { FileText, Upload, CheckCircle2, Download, Search } from 'lucide-react';
...
export default function AdditionalReportsPage() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      await reportService.uploadDoc(selectedId, selectedFile);
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="page-subtitle">
          {canSubmit ? 'View or submit post-event reports' : 'View submitted post-event reports'}
        </p>
      </div>
      ...
      {canSubmit && (
        <Alert type="info">
          ...
          <span>Submitting a report will <strong>archive</strong> the event.</span>
        </Alert>
      )}
            ...
            {loadingReport ? (
              ...
              <div className="card p-6 border-[var(--input-focus-ring)] bg-surface/30 relative space-y-4">
                <div className="flex items-start justify-between pr-8">
                  <div>
                    <h3 className="section-title">Post-Event Report</h3>
                    ...
                    {selectedFile ? (
                      ...
                      <>
                        <Button variant={reportPath ? 'secondary' : 'primary'} icon={<Upload className="w-4 h-4" />} onClick={() => fileRef.current?.click()} className="w-full justify-center h-11">
                          {reportPath ? 'Upload New Report' : 'Select Report File'}
                        </Button>

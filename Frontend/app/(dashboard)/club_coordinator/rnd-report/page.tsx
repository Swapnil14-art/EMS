...
// Lazy load the RnD report generator to keep page.tsx bundle lean
const RnDReportGenerator = dynamic(() => import('@/components/events/RnDReportGenerator'), {
  loading: () => (
...
export default function ClubCoordinatorRnDReportPage() {
  ...
  const fetchEvents = () => {
    eventService.list({ status: 'completed,archived', size: 50, manage_only: true, is_rnd: true })
  ...
  const handleConfirmUpload = async () => {
    ...
    setUploading(true);
    try {
      const result = await rndReportService.uploadDoc(selectedId, selectedFile);
      const newPath = result?.generated_report_path ?? result?.path ?? null;
      if (newPath) setUploadedReportPath(newPath);
      setUploaded(true);
      fetchEvents(); // Refresh in case status logic changes
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="page-title">RnD Report</h1>
        <p className="page-subtitle">Submit or download the separate RnD report for this event</p>
      </div>

      <Alert type="info">
        <FileText className="w-4 h-4" />
        <span>Submitting the RnD report does not affect the event lifecycle status. You can upload a pre-made report OR generate one inline.</span>
      </Alert>
        ...
        {!selectedId ? (
          <div className="space-y-4">
            <div className="relative">
              <input 
                type="text"
                placeholder="Search completed/archived events by name..." 
                value={searchInput}
                onChange={(e) => {
            ...
            {filteredEvents.length === 0 ? (
              <div className="card p-8 text-center text-[var(--text-muted)]">No completed or archived events found</div>
            ) : (
          ...
          <div className="p-4 bg-[var(--page-bg)] rounded-xl relative border border-[var(--border-subtle)]">
            ...
            {reportFilePath && (
              <div className="mt-2 flex items-center gap-1.5 text-[var(--status-success-text)] text-xs font-semibold">
                <CheckCircle2 className="w-3.5 h-3.5" />RnD Report already submitted
              </div>
            )}
          </div>
        )}
        ...
        {loadingReport ? (
              ...
              <div className="flex items-start justify-between mb-4 pr-8">
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)] mt-1">
                    The RnD report has been submitted. You can download it or upload a new one to replace it.
                  </p>
                </div>
              ...
              <div className="border border-dashed border-[var(--input-focus-ring)] bg-white rounded-xl p-4 text-center">
                ...
                {selectedFile ? (
                  ...
                  <>
                    ...
                    <p className="text-[10px] text-[var(--text-muted)] mt-2">Replaces existing RnD report document.</p>
                  </>
                )}
              </div>
            </div>
          ...
          <>
            ...
            <div className="card p-6 border-[var(--input-focus-ring)] bg-surface/30 relative">
              <div className="flex items-start justify-between mb-4 pr-8">
                <div>
                  <h3 className="section-title">Upload RnD Report File</h3>
                  <p className="text-xs text-[var(--text-muted)] mt-1">
                    Upload the RnD report document.
                  </p>
                </div>
                {reportFilePath && (
                  <CheckCircle2 className="w-5 h-5 text-[var(--status-success-text)] absolute top-6 right-6" />
                ...
                {selectedFile ? (
                  ...
                  <>
                    <Button
                      ...
                      disabled={!selectedId}
                    >
                      {reportFilePath ? 'Upload New Document' : 'Select RnD Report File'}
                    </Button>
                    {reportFilePath ? (
            ...
            <div className="relative">
              ...
              >
                ...
                {showGenerator ? 'Collapse Generator' : 'Generate RnD Report'}
              ...
              {!selectedId && (
                <p className="text-xs text-center text-[var(--text-muted)] mt-1.5 italic">
                  Available after selecting a completed/archived event
                </p>
              )}
            </div>
            ...
            {showGenerator && selectedEvent && (
              <div className="pt-2">
                <RnDReportGenerator event={selectedEvent} onComplete={handleGenerateComplete} />
              </div>
            )}
          </>
        )}

...
export default function Page() {
  ...
  useEffect(() => {
    eventService.list({ size: 50 }).then((res) => {
      const completedEvent = res.data?.find((e: Event) => e.status === 'completed');
      if (completedEvent) {
  ...
  if (pendingEvent) {
    return (
      <div className="max-w-2xl mx-auto mt-10 p-8 card text-center animate-fade-in border-2 border-amber-200 bg-amber-50/30">
        ...
        <div>
          <Link href={`/club_coordinator/events/${pendingEvent.id}`} className="btn-primary inline-flex justify-center flex-1 w-full max-w-xs items-center gap-2">

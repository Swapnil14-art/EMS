...
export default function DeptClubsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div><h1 className="page-title">School Clubs</h1><p className="page-subtitle">{clubs?.length} clubs in your school</p></div>
        ...
        {loading ? Array.from({ length: 4 }).map((_, i) => (
          ...
          <div key={c.id} className="card-hover p-5">
            <div className="flex items-start gap-3 mb-3">
              ...
              <div><h3 className="font-display font-bold text-[var(--text-primary)]">{c.name}</h3><div className="text-xs text-[var(--text-muted)]"><SchoolDisplay value={c.department?.name} /></div></div>
            </div>
            {c.coordinator && (
